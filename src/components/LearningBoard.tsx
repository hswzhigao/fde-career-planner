  ...
  return (
    <div className="space-y-6">
      ...
      {PHASES.map((phase) => {
        ...
        return (
          <div key={phase.key} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-lg font-semibold text-gray-900">{phase.label}</h2>
              <span className="text-sm text-gray-500">{doneCount}/{phaseTasks.length}</span>
            </div>
            ...
            <div className="space-y-2">
              {phaseTasks.map((task) => (
                <div key={task.id} className="flex items-center gap-3 group">
                  ...
                  <span className="text-xs text-gray-400">{CAT_LABELS[task.category] || task.category}</span>
                  <button
              ...
              {phaseTasks.length === 0 && <p className="text-sm text-gray-400">暂无任务</p>}
            </div>
          </div>
        );
      })}
  ...
  return (
    <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 space-y-4">
      ...
      <div className="flex gap-4">
        <select value={phase} onChange={(e) => setPhase(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm">
          <option value="30">30 天</option>
          <option value="60">60 天</option>
          <option value="90">90 天</option>
        </select>
        <select value={category} onChange={(e) => setCategory(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm">
          <option value="delivery">客户交付</option>
          <option value="ai_engineering">AI 工程</option>
          <option value="business">业务理解</option>
        </select>
        <select value={priority} onChange={(e) => setPriority(e.target.value)} className="px-3 py-2 border border-gray-300 rounded-md text-sm">
          <option value="high">高</option>
          <option value="medium">中</option>
          <option value="low">低</option>
        </select>
        <button onClick={submit} className="px-4 py-2 bg-brand-600 text-white rounded-md text-sm font-medium">
          添加
        </button>
      </div>
    </div>
